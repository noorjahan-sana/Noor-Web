...
function DashboardTab() {
    ...
    const { mode, product, edithandle, deleteProduct, user, orders, orderhandle, orderType, setorderType } = context
    // console.log(product)
                        ...
                        <TabPanel>
                            ...
                            <div className="relative overflow-x-auto mb-16">
                                ...
                                <div className=" flex justify-end">
                                    ...
                                    <select value={orderType} onChange={(e) => { setorderType(e.target.value) }} className="px-4 py-3 lg:w-1/6 w-1/3 rounded-md bg-gray-50 border-transparent outline-0 focus:border-gray-500 focus:bg-white focus:ring-0 text-sm" style={{ backgroundColor: mode === 'dark' ? 'rgb(64 66 70)' : '', color: mode === 'dark' ? 'white' : '', }}>
                                        ...
                                        <option key={'Packing completed'} value={'Packing completed'}>Packing completed</option>
                                        <option key={'Order completed'} value={'Order completed'}>Order completed</option>
                                        <option key={'all'} value={''}>All</option>
                                    </select>

                                    <FaFilter size={20} className='mt-3' />
                                ...
                                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400" >
                                    ...
                                    {/* for order filters */}
                                    {orderType?  orders.filter((obj) => obj.status === orderType).map((item, index) => {
                                        const { name, adress, pincode, number, date, status } = item;
                                        ...
                                        return (
                                            <tbody key={index}>
                                                ...
                                                <tr className="bg-gray-50 border-b  dark:border-gray-700" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }} >
                                                    ...
                                                    <td className="px-6 py-4 text-black " style={{ color: mode === 'dark' ? 'white' : '' }}>
                                                        <Link to={'/updateorder'} onClick={() => { orderhandle(item) }} >
                                                            <div  >
                                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                                                                    <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                                                </svg>
                                    ...
                                    {orderType? '': orders.map((item, index) => {
                                        const { name, adress, pincode, number, date, status } = item;
